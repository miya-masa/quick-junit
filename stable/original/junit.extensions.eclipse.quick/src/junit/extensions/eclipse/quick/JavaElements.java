...
public class JavaElements {
  ...
  public static boolean hasTestAnnotationRecursive(IType type) throws JavaModelException {
    if (hasTestAnnotation(type)) {
      return true;
    }
    IJavaElement[] elements = type.getChildren();
    for (Iterator iterator = Arrays.asList(elements).iterator(); iterator.hasNext();) {
  ...
  private static boolean hasTestAnnotationOnMethod(IMethod method) throws JavaModelException {
    return method.getSource() == null ? false : method.getSource().indexOf("@Test") > -1
        || method.getSource().indexOf("@Theory") > -1;
  }

  private static boolean isStaticSuiteMethod(IMethod method) throws JavaModelException {
    return ((method.getElementName().equals("suite")) && method.getSignature().equals("()QTest;")
