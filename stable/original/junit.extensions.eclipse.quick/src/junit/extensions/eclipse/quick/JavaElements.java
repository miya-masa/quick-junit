...
public class JavaElements {
	public static boolean isTestMethod(IJavaElement element)
		...
		while (element != null) {
			...
			if (isTestRunnerPassibleClass(element)) {
				...
				if (hasSuiteAnnotation(type))
					return element;
				if (hasTestAnnotationRecursive(type))
					return element;
			}
	...
	private static boolean hasParameterizedAnnotation(IType type)
			throws JavaModelException {
	...
	private static boolean hasSuiteAnnotation(IType type)
			throws JavaModelException {
		String source = type.getSource();
		if (source == null)
			return false;
		return source.indexOf("@SuiteClasses") != -1
				&& source.indexOf("Suite.class") != -1;
	}

	private static boolean isTestRunnerPassibleClass(IJavaElement element)
			throws JavaModelException {
	...
	public static boolean hasTestAnnotationRecursive(IType type)
			throws JavaModelException {
		if (hasTestAnnotation(type)) {
			return true;
		}
		IJavaElement[] elements = type.getChildren();
		for (Iterator iterator = Arrays.asList(elements).iterator(); iterator
				.hasNext();) {
			IJavaElement child = (IJavaElement) iterator.next();
			if (child instanceof IType) {
				if (hasTestAnnotation((IType) child)) {
					return true;
				}
			}
		}
	...
	private static boolean hasTestAnnotation(IType type)
			throws JavaModelException {
		IMethod[] methods = type.getMethods();
		for (int i = 0; i < methods.length; i++) {
	...
	private static boolean hasTestAnnotationOnMethod(IMethod method)
			throws JavaModelException {
		return method.getSource() == null ? false : method.getSource().indexOf(
				"@Test") > -1;
	}

	private static boolean isStaticSuiteMethod(IMethod method)
			throws JavaModelException {
		return ((method.getElementName().equals("suite"))
				&& method.getSignature().equals("()QTest;")
				&& Flags.isPublic(method.getFlags()) && Flags.isStatic(method
				.getFlags()));
	}
}
